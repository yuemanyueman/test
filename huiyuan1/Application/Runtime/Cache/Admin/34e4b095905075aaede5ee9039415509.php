...
<div class="table-box">
	<table>
        ...
        <?php if(is_array($data)): foreach($data as $k=>$vo): ?><tr>
                ...
                <td class="name"><?php echo ($vo["p_name"]); ?></td>
                <td class="type">
                	<a class='show'><?php echo ($vo["p_model"]); ?></a>
                </td>
				<td class="descript">
					<a href='' class="content" data="<?php echo ($vo["p_descript"]); ?>">查看全文</a>
				</td>
                <td class="addtime"><?php echo ($vo["p_addtime"]); ?></td>
                <td class="oltime"><?php echo ($vo["p_oltime"]); ?></td>                
            	<td >
            	<?php if($vo["p_status"] == 1): ?>已上线
            	<?php elseif($vo["p_status"] == 2): ?>待上线
            	<?php else: ?>已下线<?php endif; ?>
				</td>
                <td class="operate">
                	<a href="<?php echo U('edit', 'id='.$vo['p_id']);?>">编辑</a>
                	<a href="<?php echo U('del', 'id='.$vo['p_id']);?>" onclick="return del()">删除</a>
                </td>
